...
void main() {
  ...
  gl_FragColor = vec4(vColor * uBrightness, vFade * a * uOpacity * uAppear);
    ...
    atmoGeom.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    atmoGeom.setAttribute('size', new THREE.BufferAttribute(sizes, 1));
    atmoGeom.setAttribute('seed', new THREE.BufferAttribute(seeds, 1));
...
void main(){
  ...
  gl_FragColor = vec4(bg + flame*uFlameAmt + texture2D(bloomTexture, vUv).xyz + texture2D(torusTexture, vUv).xyz + texture2D(tDiffuse, vUv).xyz, 1.);
    ...
    const handleScroll = () => {
      ...
      if (max > 0) scrollTarget = clamp(window.scrollY / max, 0, 1);
    };
    ...
    const handleBreatheToggle = (e: any) => {
      isBreathing = e.detail?.active ?? !isBreathing;
      if (isBreathing) breathStart = performance.now() / 1000;
    };
    window.addEventListener('breathe-sync', handleBreatheToggle);
    ...
    const animate = () => {
      ...
      stream += 0.016 * (flow * 2.0) * 4.0;
      uniforms.uStream.value = stream;

      let currentWaveHeight = waveHeight;
      let targetCamY = camStartY;

      if (isBreathing) {
        // 10 second breath cycle: 4s in, 6s out
        const cycle = (t - breathStart) % 10.0;
        let breathPhase = 0; // 0 to 1
        
        if (cycle < 4.0) {
          // Inhale (0 to 4s)
          breathPhase = Math.sin((cycle / 4.0) * (Math.PI / 2));
        } else {
          // Exhale (4 to 10s)
          breathPhase = Math.cos(((cycle - 4.0) / 6.0) * (Math.PI / 2));
        }

        currentWaveHeight = waveHeight + (breathPhase * 3.5); // Wave swells up
        targetCamY = camStartY - (breathPhase * 1.5); // Camera dips slightly closer
      }
      
      uniforms.uWaveHeight.value = Lerp(uniforms.uWaveHeight.value, currentWaveHeight, 0.05);
      const smoothCamY = Lerp(camera.position.y, targetCamY + mouse.y * parallax * 0.3, 0.05);
      
      camera.position.set(mouse.x * parallax, smoothCamY, camStartZ);
      camera.lookAt(mouse.x * parallax * 0.5, 0.0, lookStartZ);
    ...
    return () => {
      ...
      window.removeEventListener('breathe-sync', handleBreatheToggle);
